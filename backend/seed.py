...
def seed():
    ...
    try:
        ...
        log_entries = [
            (created_apps[0], created_customers[0], "客户自助提交", "客户通过自助链接提交基本信息", None),
            (created_apps[1], created_customers[1], "客户自助提交", "客户通过自助链接提交基本信息", None),
            (created_apps[0], created_customers[0], "上传材料", "上传: 身份证正反面扫描件.pdf", salesman1_id),
            (created_apps[0], created_customers[0], "上传材料", "上传: 学士学位证书.pdf", salesman1_id),
            (created_apps[1], created_customers[1], "上传材料", "上传: 身份证正反面扫描件.pdf", salesman1_id),
            (created_apps[2], created_customers[2], "提交评审机构", "提交至: 广东省人力资源和社会保障厅", salesman1_id),
            (created_apps[4], created_customers[4], "提交评审机构", "提交至: 杭州市人力资源和社会保障局", salesman2_id),
        ]

        for app, customer, action, detail, actor_id in log_entries:
            log = OperationLog(
                application_id=app.id,
                customer_id=customer.id,
                action=action,
