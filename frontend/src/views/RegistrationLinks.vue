    ...
    <el-table :data="tokens" v-loading="loading" style="width: 100%">
      <el-table-column label="专属链接" min-width="300">
        <template #default="{ row }">
          <div class="link-cell">
            <el-input :model-value="getFullLink(row.token)" readonly size="small" style="width: 100%">
              <template #append>
                <el-button @click="copyLink(row.token)">
      ...
      <el-table-column label="使用次数" width="120">
        <template #default="{ row }">
          <span v-if="row.max_uses === 0">无限</span>
    ...
    <el-dialog v-model="showCreateDialog" title="生成专属注册链接" width="480px">
      <el-form label-position="top">
        <el-form-item label="使用次数限制">
          <el-radio-group v-model="createForm.maxUses">
        ...
        <el-form-item label="有效期">
          <el-select v-model="createForm.expiresDays" style="width: 100%">
...
const createForm = ref({ maxUses: 0, expiresDays: 7 })
